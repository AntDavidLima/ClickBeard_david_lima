...
export class PostgresAppointmentRepository implements AppointmentRepository {
  async save(appointment: Appointment) {
    ...
    try {
      ...
      const result = await client.query('SELECT * FROM appointments WHERE id = $1', [appointment.id]);
  ...
  async findByBarberAndAppointmentTimeBetween(barberId: UUID, startTime: Date, duration: number) {
    ...
    const sql = "SELECT (appointment_time, appointment_time + $1::interval) OVERLAPS ($2::timestamp, $2 + $1::interval) FROM appointments WHERE barber_id = $3";
